...
public class MetaTileEntityPrimitiveMudPump extends MultiblockControllerBase implements IPrimitivePump {
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('A' as char, frames(Materials.TreatedWood))
                .where('B' as char, states(MetaBlocks.PLANKS.getState(BlockGregPlanks.BlockType.TREATED_PLANK)))
                .where('C' as char, states(MetaBlocks.METAL_CASING.getState(BlockMetalCasing.MetalCasingType.BRONZE_BRICKS)).or(abilities(MultiblockAbility.PUMP_FLUID_HATCH)).or(metaTileEntities(MetaTileEntities.FLUID_EXPORT_HATCH[0], MetaTileEntities.FLUID_EXPORT_HATCH[1])))
